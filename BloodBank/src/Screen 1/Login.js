...
const Signup = ({ navigation }) => {
    ...
    return (
        <View className="h-full w-full" style={{ backgroundColor: '#fff' }}>
            ...
            <TouchableOpacity onPress={loginAccount} style={{ borderWidth: 2, borderColor: '#D70032' }} activeOpacity={0.9} underlayColor="red" className="bg-white rounded-xl w-3/4 mx-auto p-5 mt-20 ">
                <Text className="text-center font-semibold tracking-wider text-xl" style={{ color: '#D70032' }}>Login</Text>
            </TouchableOpacity>
            <View className="flex flex-row p-10 py-0">
                <TouchableOpacity activeOpacity={0.8} className="rounded-md mt-4" onPress={() => navigation.navigate("Signup")}>
                    <View className="flex flex-row mr-auto p-3">
                        <Text className="text-right  text-base" style={{ color: '#D70032' }}>Create An Account </Text>
                    </View>
                </TouchableOpacity>
            </View>
            {error == false ? <></> :
                <View className="w-5/6 border border-red-600 p-4 mx-auto mt-16 rounded bg-red-100 flex flex-row">
                    ...
                    {errorMsg == "auth/wrong-password" ?
                        <Text className="text-red-600 my-auto mx-auto">Wrong Password! Try Again</Text>
                        : <></>
                    }
                    {errorMsg == "auth/invalid-email" ?
                        <Text className="text-red-600 my-auto mx-auto">Account with that email doesn't exist</Text>
                        : <></>
                    }
                </View>
            }
